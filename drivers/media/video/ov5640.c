...
static int ov5640_config_timing(struct v4l2_subdev *sd)
{
	...
	printk("%s() -> i:%d w:%d h:%d\n", __func__, i, ov5640->format.width, ov5640->format.height);
...
static int ov5640_init(struct v4l2_subdev *subdev)
{
...
static int ov5640_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		...
		if (ret)
		{
			dev_err(&client->dev, "failed to init ov5640 (%d)\n", ret);
			ov5640_s_power(sd, 0);
			return ret;
		}
...
#if 0
		printk("Setting All in One table registers\n");
	        ret = ov5640_reg_writes(client, all_in_one,
				ARRAY_SIZE(all_in_one));
		if (ret) {
			printk("%s(failure on %d) \n", __func__, __LINE__);
			goto out;
		}
		printk("Successfully configured All in One table registers.\n");
#endif
