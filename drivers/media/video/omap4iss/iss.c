...
static int iss_probe(struct platform_device *pdev)
{
	...
	printk("Revision %08x found\n", iss->revision);
	printk("Revision %08x found\n", iss->revision);
	printk("Revision %08x found\n", iss->revision);
	printk("Revision %08x found\n", iss->revision);

	for (i = 1; i < OMAP4_ISS_MEM_LAST; i++) {
		dev_info(iss->dev, "omap4-mem %d found\n", i);
		printk("omap4-mem %d found\n", i);
...
static int __init iss_driver_init(void)
{
        return platform_driver_register(&iss_driver);
}
module_init(iss_driver_init);

static void __exit iss_driver_exit(void)
{
        platform_driver_unregister(&iss_driver);
}
module_exit(iss_driver_exit);
#endif
